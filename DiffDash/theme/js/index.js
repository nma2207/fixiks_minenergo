...
//  $('path').not(this).css('fill','rgba(0,0,0,0.5)');
  ...
  $(this).css('fill','rgba(0,0,0,0.2)');
});

$('path').bind('click',function(e){
   window.location.href = "http://127.0.0.1:5000/reg/"+ $(this).attr('id')
})
...
$('.reg').hover(function(e) {
  ...
  $('.indicator').html('');
  var name = $(idHover).attr('name').toUpperCase();
  $('<div>' + name +'</div>').appendTo('.indicator');
  //  $('path').not(this).css('fill','rgba(0,0,0,0.5)');
  ...
  $('.indicator').html('');
  $('.indicator').hide();
  $(idHover).css('fill','rgba(0,0,0,0.2)');
});
